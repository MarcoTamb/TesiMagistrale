...
\begin{theorem}
	Let $u$ satisfy the uniformly parabolic differential inequality (\ref{condition_max_principle}) with $c(x)\geq 0$
...
\begin{theorem}
	...
	\begin{align*}
		Lu\geq 0
	\end{align*} 
	with $L$ a differential parabolic operator with bounded coefficients in a domain $E$, and let $E_t = \{(x, s) \in E | s \leq t\}$. Suppose the maximum $M$ of $u$ is attained at a point $P=(x, t)$ on the boundary $\partial E$. \\
...
\begin{proof}
	...
	Thus, in the region $R$, $v(y, s)\geq0$ and has a minimum point at the boundary on $v(x, t)=0$.
	
	We can also compute $Lv$. After some calculation, we get that 
	\begin{align*}
		Lv = 2 \alpha e^{\left(-\alpha(s-t_0)^2-\sum_{i=1}^n\alpha(y_i-z_i)^2\right)}[2\, \alpha\, (y-z&)^t A (y-z) \, a_{ij} +\\
		+&\sum_{i}^n[ b_i (y_i-z_i)+a_{i,i}]+(s-t)]
	\end{align*}
	where $A$ is the matrix of the $a_{i,j}$. In particular, one can choose large enough $\alpha$ so that $Lv>0$ in $R\cup \partial R$.
...
{\vspace{10pt}\LARGE \bf [L'ipotesi $c(x) \geq 0$ cosa diventa con la non-linearità?]}
\begin{comment}
\section{A step from Gidas-Ni-Niremberg}

{\LARGE \bf [DA RIGUARDARE ALLA LUCE DELLE IPOTESI CHE CI SERVONO]}

We can now introduce an important observation used in \cite{GidasNirenberg} that allows us to apply the maximum principle \ref{maxprincprotterweinberger} and Hopf's boundary point lemma \ref{HopfBPL} in some non-linear settings. 
...
{\vspace{10pt}\LARGE \bf [DA RIVEDERE SE EFFETTIVAMENTE RISOLVE IL PROBLEMA O CI SERVE ALTRO]}

\begin{theorem}[Theorem of the mean's trick]\label{Gidas_trick}
\end{theorem}
...
The paper from Huisken and Polden \cite{huisken} proves the following comforting existence result for the class of equations we are analysing under very broad hypothesis, which we will not use, nor prove, but which we include for completeness: 
