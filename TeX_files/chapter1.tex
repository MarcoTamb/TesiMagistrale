...
{\vspace{10pt}\LARGE \bf [Definizione esplicita di $h_{ij}$?]}
...
\begin{oss}\label{removecpositive}
	...
	\begin{align*}
		Lv-\lambda v \geq 0
	\end{align*}
	whenever $Lu\geq 0$, and we can chose $\lambda$ large enough such that $c(x)-\lambda<0$ and thus we can remove the hypothesis $c(x)\leq0$ in both theorems when $c$ is bounded. 
\end{oss}
...
\begin{proof}
	...
	\end{align*}
	for a positive $\alpha\in [0, +\infty)$, where we are taking $V$ as outward pointing normal to the boundary, because the graphs are both tangent to $T_xM_t$ and they have non-positive derivative, as the point is a minimum on the boundary. At the same time, 
	\begin{align*}
