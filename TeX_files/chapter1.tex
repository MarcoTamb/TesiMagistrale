...
\begin{theorem}[Maximum principle for parabolic equations]\label{maximum_principle}
	...
	\end{align}
	where $L$ is an elliptic differential operator, i.e. there exists $\theta>0$ such that $\sum_{i,j=1}^{n} a_{ij}(x) \xi_i\xi_j \geq \theta |\xi |$ for all $\xi \in \R^n$ and $x \in \Omega$. Suppose also that $c\equiv 0$ in $\Omega$. Then: \begin{itemize}
...
We can now introduce an important observation shown in \cite{protterweinberger} that allows us to apply the maximum principle \ref{maxprincprotterweinberger} and Hopf's boundary point lemma \ref{HopfBPL} in some non-linear settings. 
\begin{comment}
...
\end{comment}
First, we remind the reader of the following generalized version of the theorem of the mean:
\begin{theorem}
...
\begin{align}
	Lu= F\left(t, x, u, \frac{\partial u}{\partial x_i} , \frac{\partial^2 u}{\partial x_i \partial x_j}\right)-u_t = f(x, t)\label{nonlinearexample}
\end{align}
...
\begin{align*}
	Lv= \left( F\left(t, x, u, \frac{\partial u}{\partial x_i} , \frac{\partial^2 u}{\partial x_i \partial x_j}\right) - F\left(t, x, w, \frac{\partial w}{\partial x_i} , \frac{\partial^2 w}{\partial x_i \partial x_j}\right)\right)-v_t \leq 0
\end{align*}

Now, we can apply Lagrange's theorem to $F$ to get 

\begin{align*}
	Lv= \left\langle \nabla F(\xi), \left(t, x, v, \frac{\partial v}{\partial x_i} , \frac{\partial^2 v}{\partial x_i \partial x_j}\right) \right\rangle-v_t \leq 0
\end{align*}
for a fixed $\xi$. Thus, the difference $v$ of two sub-solutions to a non-linear parabolic problem is a sub-solution to a (different) \textit{linear} parabolic problem, as the derivatives of $F$ and $\xi$ do not depend on $v$ ($\xi$ can be chosen a-priori). We can thus apply the maximum principle and the Hopf's boundary point lemma to $v$. This can allow us to state the following two results which we will be using later:

\begin{proposition}[Maximum principle for parabolic non-linear differential equations]
	\label{firstapplication}
	Suppose we have two solution $u$ and $w$ in the interval $[0, T]$ to the same parabolic non-linear differential equation (\ref{nonlinearexample}) on an open set $\Omega$, but with different start conditions: then, if $u>w$ in the interior of $\Omega$ at $t=0$ and $u\geq w$ on $\partial\Omega$,  $u>w$ for all $t\in[0, T]$ in the interior of $\Omega$.
\end{proposition}

\begin{proof}
	$v=u-w\geq 0$ is a solution of a parabolic linear differential equation. At $t=0$, $v>0$ in the interior of $\Omega$. If at an interior point $x$ $u=w$ at a certain time $t=\tau$, $v(\tau, x)=0$, and thus $v$ is not constant. However, it attains minimum ($v=0$) at an interior point, thus by \ref{maximum_principle} it must be constant, a contradiction. 
\end{proof}

\begin{proposition}[Hopf's boundary point lemma for parabolic non-linear differential equations]
\label{secondapplication}
Suppose we have two solution $u$ and $w$ in the interval $[0, T]$ to the same parabolic non-linear differential equation (\ref{nonlinearexample}) on an open set $\Omega$, but with different start conditions. Let $v=u-w$: then, 
\begin{align*}
	\frac{\partial v}{\partial \nu} >0
\end{align*}
at all points in $\partial\Omega$, where we take $\nu$ as the normal to $\partial\Omega$. 
\end{proposition}

\begin{proof}
	{\vspace{10pt}\LARGE \bf [SCRIVERE PROOF alla luce di fix a teorema \ref{HopfBPL}]}
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	for appropriate functions $\tilde{f}:U\times (t-\varepsilon, t+\varepsilon)\rightarrow \R$ and $\tilde{f}_\Pi:U\times (t-\varepsilon, t+\varepsilon)\rightarrow \R$, where $\nu$ is a fixed unit vector normal to $T_{x_0}M_{t}$.  $\tilde{f}$ and $\tilde{f_\Pi}$ are solutions to the same second order PDE, which is parabolic by what was discussed in paragraph \ref{parabolic}, hence we can apply \ref{firstapplication} to conclude that $\tilde{f}\equiv\tilde{f_\Pi}$, and thus $M_{t,\Pi}$ and $M_{t}$ coincide in a neighbourhood of $(x, t)$. \\ Iterating the procedure on all boundary points of this neighbourhood, we find that the largest set on which they coincide must be an open set, but manifolds can only coincide on closed sets, and thus the two manifolds must completely coincide for all $\tau\leq T$, a contradiction.
	...
	\begin{align*}
		...
		f_\Pi \; : \; (x, t) &\mapsto x+\tilde{f}_\Pi(x, t)\nu 
	\end{align*} 
	Moreover, in $\overline{H^-(\Pi)}$, $f_\Pi\geq f$, because $M^n_\Pi\cap H^-(\Pi)\subset \mathrm{int}(M^n)\cap H^-(\Pi)$. Finally, $f(x, t)=f_\Pi (x, t)$, and $f(x, t)$ is a minimum point for both functions. Also, we must have
	\begin{align*}
		\frac{\partial f}{\partial V}(x,t)=\alpha\frac{\partial f_\Pi}{\partial V}(x,t)
	\end{align*}
	for a positive $\alpha\in [0, +\infty)$, where we are taking $V$ as outward pointing normal to the boundary, because the graphs are both tangent to $T_xM_t$ and they have nonpositive derivative, as the point is a minimum on the boundary. At the same time, 
	\begin{align*}
		\frac{\partial f}{\partial V}(x,t)=-\frac{\partial f_\Pi}{\partial V}(x,t)
	\end{align*}
	by reflection. Hence $\frac{\partial f}{\partial V}(x,t)=\frac{\partial f_\Pi}{\partial V}(x,t)=0$. Thus, 
	\begin{align*}
	...
	\begin{align*}
		\frac{\partial (f- f_\Pi)}{\partial V}(x,t)>0
	\end{align*}
	at a minimum on the boundary by \ref{secondapplication}, a contradiction.  
\end{proof}
