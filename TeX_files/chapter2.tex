...
{\Large \textbf{[Ancient solutions - DA SCRIVERE]}}

\begin{defin}
	We say that a solution to \ref{evolutioneq} is {\em expansive} if $F<0$. 
\end{defin}
\begin{oss}	
	This assumption on $F$ implies that the flow is  {\em expansive}, in the sense that whenever $t<s$, $X_t \subset \mathrm{int}(X_s)$. This is implied by the fact that, in the equation, the time derivative is always an outward pointing non-zero vector.\label{expansive flow remark}
\end{oss}
